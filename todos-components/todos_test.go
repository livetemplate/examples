...
func TestTodosComponentsE2E(t *testing.T) {
	...
	t.Run("Delete Confirmation Modal Appears", func(t *testing.T) {
		...
		err := chromedp.Run(modalCtx,
			chromedp.Evaluate(`document.querySelector('input[name="id"][value="2"]').closest('form').querySelector('button[name="confirm_delete"]').click()`, nil),
	...
	t.Run("Cancel Delete Modal", func(t *testing.T) {
		...
		if err != nil || !modalVisibleBefore {
			...
			chromedp.Run(modalCtx,
				chromedp.Evaluate(`document.querySelector('button[name="confirm_delete"]').click()`, nil),
				e2etest.WaitFor(`document.querySelector('[data-modal="delete_confirm"]') !== null`, 5*time.Second),
			)
		}
	...
	t.Run("Confirm Delete Modal Deletes Todo", func(t *testing.T) {
		...
		err := chromedp.Run(modalCtx,
			...
			chromedp.Evaluate(`document.querySelector('input[name="id"][value="1"]').closest('form').querySelector('button[name="confirm_delete"]').click()`, nil),
			e2etest.WaitFor(`document.querySelector('[data-modal="delete_confirm"]') !== null`, 5*time.Second),
		)
		if err != nil {
			t.Fatalf("Failed to open delete modal: %v", err)
	...
	t.Run("Clear Completed Removes Completed Todos", func(t *testing.T) {
		...
		err = chromedp.Run(clearCtx,
			...
			chromedp.Click(`button[name="clear_completed"]`, chromedp.ByQuery),
			chromedp.Sleep(500*time.Millisecond),
			// Count todos after
