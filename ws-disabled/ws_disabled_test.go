...
func TestWSDisabled_PRGPattern(t *testing.T) {
	...
	form := strings.NewReader("action=add&label=Test&url=https://test.com")
	req, err := http.NewRequest("POST", server.URL, form)
	if err != nil {
		t.Fatalf("Failed to create request: %v", err)
	}
...
func TestWSDisabled_JSONResponse(t *testing.T) {
	...
	form := strings.NewReader("action=add&label=API+Test&url=https://api.example")
	req, err := http.NewRequest("POST", server.URL, form)
	if err != nil {
		t.Fatalf("Failed to create request: %v", err)
	}
...
func TestWSDisabled_DiffOptimization(t *testing.T) {
	...
	doPost := func(label, url string) string {
		form := strings.NewReader(fmt.Sprintf("action=add&label=%s&url=%s", label, url))
		req, err := http.NewRequest("POST", server.URL, form)
		if err != nil {
			t.Fatalf("Failed to create request: %v", err)
		}
